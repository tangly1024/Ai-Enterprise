...
- 区块标题优先用短名词，例如“产品价格”“交付流程”“常见问题”。
- 不要把标题写成解释句，例如“先低门槛验证，不让老板一上来背大预算”。
- 标题负责告诉访客“这是什么”，辅助说明负责解释“为什么可信”。
- 删除重复铺垫和销售腔，除非它能降低明确的购买顾虑。
- 交付物、验收标准、退款边界、数据安全、付款限制可以保留细节，因为它们建立信任。
...
移动端页面较长时，保留一个底部固定 CTA，文案只写“免费初筛”和“15 分钟判断值不值得做”。不要放倒计时或夸张承诺。
...
所有关键申请动作都要有 `data-track`：首屏 CTA、价格 CTA、适合度自检、移动端固定 CTA、邮件申请、邮件模板、方案文档。
